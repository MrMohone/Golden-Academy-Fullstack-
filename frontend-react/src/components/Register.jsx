...
const Register = () => {
  ...
  const [errors, setError] = useState({})//set as empty object
  const [Success, setSuccess] = useState(false)
  const [loading, setLoading] = useState(false)

 const handleRegistration = async (e) => {
      e.preventDefault() //when submit btn clicked, keep current value that exist in input field
      setLoading(true)
      const userData = {
        username, email, password
      }
      // console.log(userData) //befor starting to connect backend check this line
      try{
        const response = await axios.post('http://127.0.0.1:8000/api/v1/register/', userData)
        //next line of excute only after backend response 
